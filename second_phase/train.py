"""Training pipeline for Phase 2: full-sphere cascading extremes.

Downloads data, fits GARCH, transforms to Laplace margins, builds sphere events,
fits directional threshold, and trains SphericalCascadeTransformer.
"""
...
from second_phase.dataset import (
    SphereEventSequenceDataset,
    SequenceConfig,
    build_events_sphere,
...
def fit(cfg: Dict, events: Optional[Dict[str, np.ndarray]] = None):
    ...
    train_ds = SphereEventSequenceDataset(train_events, seq_cfg)
    val_ds = SphereEventSequenceDataset(val_events, seq_cfg)
    ...
    model = SphericalCascadeTransformer(d_input=d_input, d_assets=d_assets, cfg=model_cfg)
    ...
    for epoch in range(cfg["train"]["epochs"]):
        ...
        if val_loss < best_val:
            ...
            torch.save(
                ...
                str(Path(artifact_dir) / "model.pt"),
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Phase 2 Spherical Cascade Transformer.")
